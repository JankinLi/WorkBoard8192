...
void set_main_head_it(){
	main_head_flag = 1;
	main_body_flag = 0;
	HAL_UART_Receive_IT(&hlpuart1,(uint8_t*)main_head_buf,8);
}
...
void receive_main_head(){
	...
	if (data_len == -1){
		...
		main_head_flag = 0;
		main_body_flag = 1;
		HAL_UART_Receive_IT(&hlpuart1,(uint8_t*)main_body_buf, data_len);
	}
}
...
void set_down_head_it(){
	...
	HAL_UART_Receive_IT(&huart1,(uint8_t*)down_head_buf,2);
}
...
void receive_down_head(){
	...
	down_body_flag = 1;
	p_down_current = down_body_buf;
	HAL_UART_Receive_IT(&huart1,(uint8_t*)p_down_current, 1);
}
			...
			if (pos < 0 || pos >= MAIN_BUFFER_COUNT){
				...
				if (data_ptr[0] == 0x01){
					...
					if(data_ptr[1] == 0x01 ){
						...
						int len_value = compute_len(len_ptr);
						char * data_ptr = len_ptr + 4;
						HAL_UART_Transmit(&huart1,(uint8_t*)data_ptr,len_value,0xffff);
					}
					else{
						PutErrorCode(ErrorQueueHandle,0x03);
					}
	...
	for(;;)
	{
		...
		if (status == osOK) {
			if (pos == -1){
				...
				HAL_UART_Transmit(&hlpuart1,(uint8_t*)out_data_ptr,total,0xffff);
			}
		}
		osDelay(1);
	}
  ...
  /* Infinite loop */
	for(;;)
	{
		...
		if (status == osOK) {
			...
			buffer[2] = 0x0E;
			buffer[3] = code_value;
			HAL_UART_Transmit(&hlpuart1,(uint8_t*)buffer,8,0xffff);
		}
		osDelay(1);
	}
